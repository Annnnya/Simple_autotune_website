...
def main(filepath, smoothing, plot=False, write=True, scale="D:min"):
    ...
    pitch_corrected_y = autotune(y, sr, aclosest_pitch_from_scale, scale, plot, int(smoothing))
...
def process_audio():
    ...
    correction_option = request.form.get('selectedOption')
    key = request.form.get('textInput')

    smoothing_option = request.form.get('selectedOption2')

    print(smoothing_option)
    print(correction_option)
    print(key)
